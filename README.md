...
### Companies Map (apps/companies_map/)
Визуализация компаний на интерактивной карте.

**Функциональность:**
- Автоматически загружает все компании и их адреса из Bitrix24.
- Использует API Яндекс.Карт для геокодирования адресов и отображения на карте.
- Каждая компания отображается анимированной меткой.
- При клике на метку отображается название и полный адрес компании.
- Карта автоматически масштабируется, чтобы показать все найденные компании.
- Требует наличия `YANDEX_API_KEY` в файле `.env` для работы с геокодером.

### Contact Manager (apps/contact_manager/)
Управление контактами: импорт и экспорт.

**Функциональность:**
- Главная страница с выбором действия: импорт или экспорт.
- Экспорт всех контактов из Bitrix24 в форматы CSV и XLSX.
- В экспортируемый файл включаются поля: имя, фамилия, телефон, email и название компании.
- Импорт контактов из файлов CSV или XLSX.
- Автоматическая проверка на дубликаты по номеру телефона или email перед созданием.
- Возможность привязки контакта к существующей компании по ее названию.
- Предоставление детального отчета о результатах импорта: сколько контактов создано, пропущено (дубликаты) и сколько было ошибок.
...
- **openpyxl 3.1.5** - для работы с файлами XLSX
